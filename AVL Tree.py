'''
Creates an AVL tree with insertion, balance, deletion, and print methods
'''
...
class AVLMod:
    ...
    def _balance(self, T):
        ...
        if BF > 1: #left skewed
            if T.leftT.balanceFactor() > 0: #left skewed left heavy
                return self._rotateRight(T)
                
            elif T.leftT.balanceFactor() < 0: #left skewed right heavy
                T.leftT = self._rotateLeft(T.leftT)
                return self._rotateRight(T)
        
        elif BF< -1: #right skewed
            if T.rightT.balanceFactor() < 0: #right skewed right heavy
                return self._rotateLeft(T)

            elif T.rightT.balanceFactor() > 0: #right skewed left heavy
